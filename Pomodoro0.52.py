...
class PomodoroCanvas:
    def __init__(self):
        ...
        self.window = Tk()
        self.window.title("Pomodoro0.52")
    ...
    def start(self):
        ...
        while True:
            # get the current time for each animation loop
            currentTime = datetime.datetime.now()

            # compute the amount of time elapsed since the pomodoro began
            self.elapsed += currentTime - startTime  # timedelta

            # TODO make sure this works

            delta = setTimer
            remaining_minutes = "{:>2d}".format((delta - elapsed.seconds) // 60)
            remaining_seconds = "{:02d}".format((delta - elapsed.seconds) % 60)
            time_text = remaining_minutes + ":" + remaining_seconds

            completion_ratio = elapsed.seconds / (setTimer)

            # check for end condition
            if delta - elapsed.seconds == -1:
                end_sound.play()
                break
